...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6282138746029@c.us'
        ...
        const vhtearkey = 'LEUJU9ybLwAHbLqnGShv' // https://api.vhtear.com
        const barbarkey = 'LEUJU9ybLwAHbLqnGShv' // https://mhankbarbar.herokuapp.com/api
        ...
        // FUNCTION
                ...
                if(body === '~mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin KOF-BOT!', id)
                ...
                if (body === '~unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner KOF-BOT!', id)
        ...
        case '~banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner KOF-BOT!', id)
...
${desc}`)
        ...
        case '~simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin KOF-BOT!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
...
➸ *Wilayah:* +${region_name}`
        ...
        // ADMIN & OWNER
        case '~bc':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner KOF-BOT!', id)
            let msg = body.slice(4)
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) await tobz.sendText(ids, `[ KOF-BOT BROADCAST ]\n\n${msg}`)
            }
        ...
        case '~omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner KOF-BOT', id)
        ...
        case '~okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin KOF-BOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '~oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin KOF-BOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '~oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin KOF-BOT', id)
            await tobz.sendText(from,'KOF-BOT DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '~leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '~getses':
            ...
            break
        case '~KOF-BOTadmin':
            let admn = `This is list of KOF-BOT Admin\nTotal : ${adminNumber.length}\n`
        ...
        case '~addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner KOF-BOT!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin KOF-BOT!', id)
                }
            break
        case '~deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner KOF-BOT!', id)
                ...
                fs.writeFileSync('./lib/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin KOF-BOT!', id)
            break
        case '~block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner KOF-BOT!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '~unblock':
            ...
            break
        case '~ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin KOF-BOT!', id)
                for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '~help':
            ...
            break
        case '~KOF-BOTgroup':
            tobz.reply(from, `Link Group KOF-BOT : https://chat.whatsapp.com/BYvfW1Ow1D6HvkW6K80s91\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '~groupmenu':
        ...
        case '~iklan':
            ...
            break
        case '~adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin KOF-BOT', id)
        ...
        case '~snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup KOF-BOT, Link Ada Di Bio atau Bisa Mengetik ~KOF-BOTgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *~iklan*', id)
            if (command.startsWith('~')) {
